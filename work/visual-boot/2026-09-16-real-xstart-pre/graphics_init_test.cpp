...
void BurnoutPpcEntryProbe(PPCContext& ctx)
{
    // Trace only the allocation wrapper and its two known callees. At entry,
    // LR is the PPC return address supplied by the direct caller.
...
int main(int argc, char** argv)
{
    ...
    if (argc == 2 && std::strcmp(argv[1], "--boot") == 0) {
        extern bool BurnoutFullBoot; BurnoutFullBoot=true;
        ctx.r3.u64=0;ctx.r4.u64=0;
    ...
    // ExCreateThread workers remain part of the running guest until process
    // termination. Keep the guest reservation alive instead of freeing it
    // while those workers can still execute PPC code.
    return 0;
}
