            ...
            if(type==0) {
                ...
                case 0x50:
                    if(n!=2) fail("invalid SET_BIN_MASK");bin_mask=(uint64_t(d[0])<<32)|d[1];break;
                                case 0x36: {
                    if(n!=1)
                        fail("immediate indices not implemented");

                    ++draws;

                    std::printf(
                        "[GPU DRAW] number=%u initiator=%08X color_mask=%08X depth=%08X\n",
                        draws,
                        d[0],
                        registers[0x2104],
                        registers[0x2200]
                    ...
                    if(registers[0x2104]==0 &&
                       registers[0x2200]==0 &&
                       shader_has_no_memory_exports(shaders[0]) &&
                       shader_has_no_memory_exports(shaders[1])) {
                        std::printf(
                            "[GPU DRAW] masked color/depth, no shader memory exports: "
                            "no framebuffer writes\n"
                        );
                        break;
                    }

                    burnout::backend::XenosDrawState state{};

                    state.initiator = d[0];
                    state.draw_number = draws;

                    state.registers = registers;
                    state.register_count = 0x5003;

                    state.vertex_shader =
                        shaders[0].empty() ? nullptr : shaders[0].data();
                    state.vertex_shader_dwords = shaders[0].size();

                    state.pixel_shader =
                        shaders[1].empty() ? nullptr : shaders[1].data();
                    state.pixel_shader_dwords = shaders[1].size();

                    if(!burnout::backend::XenosBackendSubmitDraw(state)) {
                        fail("rendering backend rejected draw");
                    }
                ...
                case 0x3c: {
                default:
                    fail("unknown/unimplemented PM4 opcode");
                }
            }
        }
