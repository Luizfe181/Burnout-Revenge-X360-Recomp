...
if not args.boot:
    ...
    for runtime in HERE.glob("xbox_*.cpp"):
        implemented_imports.update(re.findall(r"^\s*(?:PPC_FUNC|PPC_FUNC_IMPL|PROBE_IMPORT)\(\s*(\w+)\s*\)", runtime.read_text(encoding="utf-8"), re.M))
    ...
    for name in unresolved:
        content += f'PPC_FUNC({name}) {{ std::fprintf(stderr,"[BOOT IMPORT STOP] {name}\\n"); BurnoutReportPpcState(); std::exit(6); }}\n'
