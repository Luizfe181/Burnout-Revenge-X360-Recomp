...
void BurnoutReportPpcState() {
    std::fprintf(stderr,"[THREAD STATE] id=%u entry=%08X startup=%08X active_indirect=%08X\n",diagnosticThreadId,diagnosticEntry,diagnosticStartup,diagnosticIndirect);
    if (!diagnosticContext) return;
    const auto& c=*diagnosticContext;
    std::fprintf(stderr,"[PPC STATE] LR=%08llX CTR=%08X r1=%08X r3=%08X r4=%08X r29=%08X r30=%08X r31=%08X\n",
