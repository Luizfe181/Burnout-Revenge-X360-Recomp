...
headers=max(f.stat().st_mtime_ns for f in [f for f in p.glob('*.h') if f.name!='graphics_lookup.h']+list(p.glob('*.inc')))
flags=['-std=c++17','-O0','-g','-mavx','-msse4.1','-include','stdio.h','-include','Windows.h','-I',str(p)]
def compile(src):
    dst=objects/(src.name+'.obj')
    if not dst.exists() or dst.stat().st_mtime_ns<max(headers,src.stat().st_mtime_ns,(p/'graphics_lookup.h').stat().st_mtime_ns if src.name=='graphics_init_test.cpp' else 0):
